...
//print_r($_POST);exit;
	...
	if (!empty($_POST)) {
		...
		if ($_POST['action'] == 'infoProductoEnter') {
			$producto_id = mysqli_real_escape_string($conection, $_POST['producto']);
			
			$query = mysqli_query($conection, "SELECT * FROM producto 
											   WHERE (codigo LIKE '%$producto_id%' 
											   OR descripcion LIKE '%$producto_id%' 
											   OR categoria LIKE '%$producto_id%') 
											   AND status = 1");
...
$query = mysqli_query($conection, "SELECT nit, nombre, telefono, direccion FROM cliente WHERE nit LIKE '%" . $q . "%'");
